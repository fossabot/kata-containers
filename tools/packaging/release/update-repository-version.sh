...
bump_repo() {
	...
	if [ "${repo}" == "kata-containers" ]; then
		...
		if [ "${target_branch}" == "main" ] && [[ "${new_version}" =~ "rc" ]]; then
			...
			git rm -f tools/packaging/kata-deploy/kata-deploy/base/kata-deploy-stable.yaml
			git rm -f tools/packaging/kata-deploy/kata-cleanup/base/kata-cleanup-stable.yaml
