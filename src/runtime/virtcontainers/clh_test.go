...
func newClhConfig() (HypervisorConfig, error) {
	...
	return HypervisorConfig{
		...
		NetRateLimiterBwMaxRate:       uint64(0),
		NetRateLimiterBwOneTimeBurst:  uint64(0),
		NetRateLimiterOpsMaxRate:      uint64(0),
		NetRateLimiterOpsOneTimeBurst: uint64(0),
	}, nil
}
...
func TestCloudHypervisorNetRateLimiter(t *testing.T) {
	...
	type args struct {
		bwMaxRate       uint64
		bwOneTimeBurst  uint64
		opsMaxRate      uint64
		opsOneTimeBurst uint64
	}
	...
	tests := []struct {
		...
		// Bandwidth
		{
			...
			args{
				bwMaxRate:      uint64(1000),
				bwOneTimeBurst: uint64(10000),
			},
		...
		// Operations
		{
			...
			args{
				opsMaxRate:      uint64(1000),
				opsOneTimeBurst: uint64(10000),
			},
			true,  //expectsRateLimiter
			false, // expectsBwBucketToken
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := clh.addNet(validVeth); (err != nil) != tt.wantErr {
				t.Errorf("cloudHypervisor.addNet() error = %v, wantErr %v", err, tt.wantErr)
			} else if err == nil {
				...
				if tt.expectsRateLimiter {
					...
					if tt.expectsBwBucketToken {
						...
						assert.Equal(bwBucketToken.GetSize(), int64(utils.RevertBytes(tt.args.bwMaxRate/8)))
						assert.Equal(bwBucketToken.GetOneTimeBurst(), int64(utils.RevertBytes(tt.args.bwOneTimeBurst/8)))
					}
